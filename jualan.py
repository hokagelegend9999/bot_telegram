...
async def create_trial_ssh_handler(u,c): await handle_general_script_button(u,c,'/usr/bin/bot-trial','Membuat trial SSH...','Gagal membuat trial SSH.',get_ssh_ovpn_menu_keyboard())
async def create_trial_vless_handler(u,c): await handle_general_script_button(u,c,'/usr/bin/bot-trialvless','Membuat trial VLESS...','Gagal membuat trial VLESS.',get_vless_menu_keyboard())
async def create_trial_trojan_handler(u,c): await handle_general_script_button(u,c,'/usr/bin/bot-trialtrojan','Membuat trial Trojan...','Gagal membuat trial Trojan.',get_trojan_menu_keyboard())
async def create_trial_vmess_handler(u,c): await handle_general_script_button(u,c,'/usr/bin/bot-trialws','Membuat trial VMess...','Gagal membuat trial VMess.',get_vmess_creation_menu_keyboard())
async def create_trial_shdwsk_handler(u,c): await handle_general_script_button(u,c,'/usr/bin/bot-trialss','Membuat trial Shadowsocks...','Gagal membuat trial Shadowsocks.',get_shadowsocks_menu_keyboard())

async def topup_saldo_handler(u,c):
    caption = f"💰 *TOP UP SALDO*\nSaldo Anda: <b>Rp {get_user_balance(u.effective_user.id):,.0f}</b>\n\nTransfer ke:\n[INFO REKENING ANDA]\n\nKonfirmasi ke: @{TELEGRAM_ADMIN_USERNAME}"
    keyboard = get_admin_main_menu_keyboard() if is_admin(u.effective_user.id) else get_main_menu_keyboard()
...
async def start_account_creation(u,c,srv,cost,next_st,kbd):
    if not is_admin(uid := u.effective_user.id) and (bal := get_user_balance(uid)) < cost:
        await u.message.reply_text(f"🚫 Saldo Tdk Cukup! Rp{bal:,.0f}<Rp{cost:,.0f}", reply_markup=kbd, parse_mode='HTML'); return ConversationHandler.END
    await u.message.reply_text(create_conversation_prompt(f"📝 Masukkan *Username* utk {srv}:"), parse_mode='HTML'); return next_st
...
# --- FUNGSI-FUNGSI CONVERSATION YANG HILANG SEBELUMNYA ---
...
async def add_balance_get_amount_step(u,c):
    ...
    target_id = c.user_data['target_user_id']; amount = float(amount_str)
...
async def create_akun_ssh_start(u,c): return await start_account_creation(u,c,"SSH",ACCOUNT_COST_IDR,SSH_OVPN_GET_USERNAME,get_ssh_ovpn_menu_keyboard())
...
# ... (Fungsi conversation steps untuk Vmess, Vless, SS, dll. akan ada di sini) ...

async def restore_vps_start(u,c):
    if not is_admin(u.effective_user.id): return ConversationHandler.END
    warning_text = "⚠️ *PERINGATAN!* ⚠️\nProses ini akan menimpa data. Tidak bisa dibatalkan.\n\nKirimkan **link download** file `backup.zip` Anda."
    await u.message.reply_text(create_conversation_prompt(warning_text), parse_mode='HTML'); return GET_RESTORE_LINK
...
def main() -> None:
    ...
    # CONVERSATION HANDLERS (DEFINISI LENGKAP DAN BENAR)
    ...
    # (Di sini seharusnya ada definisi conv handler untuk vmess, vless, ss, check balance, dll)

    application.add_handler(ssh_conv)
    application.add_handler(add_balance_conv)
    application.add_handler(restore_conv)
    # (Di sini seharusnya ada application.add_handler untuk semua conv handler lainnya)
    ...
    for regex, func in handlers.items():
